...
const REUSE_PENDING_REPLY_MS = 2 * 60 * 1000
...
function clearInFlightMappingById(msgId: string) {
  for (const [message, mappedId] of inFlightCodexMessages) {
    if (mappedId === msgId) inFlightCodexMessages.delete(message)
  }
}
...
function abandonPendingReply(msgId: string) {
  const pending = pendingReplies.get(msgId)
  if (!pending) {
    clearInFlightMappingById(msgId)
    return false
  }
...
function shouldReusePendingReply(pending: PendingReply) {
  if (pending.reply !== undefined) return true
  if (pending.waiters.size > 0) return true
  return Date.now() - pending.createdAt <= REUSE_PENDING_REPLY_MS
}
...
function resolveCodexReply(replyToId: string | undefined, text: string) {
  let targetId = replyToId
  let pending = targetId ? pendingReplies.get(targetId) : undefined

  // Claude is instructed to pass reply_to, but if it slips and there is
  // exactly one live Codex request, route the reply there instead of making
  // Codex burn the full timeout while the answer is already visible in UI.
  if (!pending) {
    const live = Array.from(pendingReplies.entries()).filter(
      ([, p]) => p.reply === undefined,
    )
  ...
  if (!targetId || !pending) return undefined
  ...
  try {
    switch (req.params.name) {
      case 'reply': {
        ...
        // Resolve the exact pending Codex request identified by reply_to.
        const resolvedReplyTo = resolveCodexReply(replyTo, text)

        const id = nextId('claude-')
        broadcast({ type: 'msg', id, from: 'claude', text, ts: Date.now(), replyTo: resolvedReplyTo ?? replyTo, file })
  ...
  fetch(req, server) {
    ...
    if (url.pathname === '/api/from-codex' && req.method === 'POST') {
      return (async () => {
        ...
        const existingPending = existingId ? pendingReplies.get(existingId) : undefined

        if (existingId && existingPending && shouldReusePendingReply(existingPending)) {
          return Response.json({ id: existingId })
        }

        if (existingId && existingPending) {
          dropPendingReply(existingId)
        } else if (existingId) {
    ...
    if (url.pathname.startsWith('/api/poll-reply/') && req.method === 'GET') {
      ...
      const requestedTimeout = Number(url.searchParams.get('timeout') ?? 120000)
    ...
    // ── API: Codex abandons a pending wait after its own timeout ──
    // DELETE /api/pending-reply/:id
    if (url.pathname.startsWith('/api/pending-reply/') && req.method === 'DELETE') {
      const msgId = decodeURIComponent(url.pathname.slice('/api/pending-reply/'.length))
      if (!msgId) return new Response('missing id', { status: 400 })
      const abandoned = abandonPendingReply(msgId)
      return Response.json({ ok: true, abandoned })
    }
    ...
    if (url.pathname === '/api/health') {
      return Response.json({
        ...
        pendingForCodex: pendingForCodex.length,
