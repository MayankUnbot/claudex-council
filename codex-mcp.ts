...
async function abandonPendingReply(id: string) {
  try {
    await fetchWithTimeout(
      `${BRIDGE_URL}/api/pending-reply/${encodeURIComponent(id)}`,
      { method: 'DELETE' },
      CHECK_TIMEOUT_MS,
    )
  } catch {
    // Best effort only. The bridge also prunes stale requests server-side.
  }
}
    ...
    switch (req.params.name) {
      case 'send_to_claude': {
        ...
        const requestPromise: Promise<ToolResult> = (async () => {
          ...
          const sendRes = await fetchWithTimeout(`${BRIDGE_URL}/api/from-codex`, {
            method: 'POST',
          ...
          await abandonPendingReply(id)
          return {
            content: [{
              type: 'text',
              text: `Claude did not reply within ${formatElapsedMs(startedAt)}. I released this wait so a retry can send a fresh request instead of reusing a stale one.`,
            }],
          }
        })()
      ...
      case 'check_claude_messages': {
        const res = await fetchWithTimeout(`${BRIDGE_URL}/api/pending-for-codex`, {}, CHECK_TIMEOUT_MS)
